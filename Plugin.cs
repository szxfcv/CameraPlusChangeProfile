...
namespace CameraPlusChangeProfile
{
    ...
    public class Plugin
    {
        ...
        private class WaitForMainMenu : MonoBehaviour
        {
            ...
            */
            //private readonly CameraPlusChangeProfileMenu _profileMenu = BeatSaberUI.CreateViewController<CameraPlusChangeProfileMenu>();
            private void OnMainMenuActivated(bool firstActivation, bool addedToHierarchy, bool screenSystemEnabling)
            {
                /*
                if (!firstActivation)
                ...
                */
                ...
                var parent = mainMenu.transform;

                // ★ ViewController を UI 階層に正しく生成
                var go = new GameObject("CameraPlusChangeProfileMenu");
                go.transform.SetParent(parent, false);

                // ★ ここで ViewController として生成される
                var menu = go.AddComponent<CameraPlusChangeProfileMenu>();

                Plugin.Instance.menu = menu;

                Plugin.Log.Info("Adding GameplaySetup tab...");

                // ★ host は必ず ViewController（menu）を渡す
                GameplaySetup.Instance.AddTab(
                    "Banao Mod",
                    "CameraPlusChangeProfile.UI.xav7ze_tab.bsml",
                    menu
                ...
                */
                ...
                try
                {
                    ...
                    //"CP Profile Change",
                    "CPCP",
                    "CameraPlusChangeProfile.UI.cpcp_tab.bsml",
                    Plugin.Instance.menu
                    );
                    Log.Info("cpcp_mod tab added safely!");
                }
                catch (Exception ex)
                {
